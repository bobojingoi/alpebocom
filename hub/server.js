...
const store = hasDb
  ...
  : {
      ...
      async posts(all) {
        ...
        return all ? list : list.filter((p) => p.published).map(({ body, ...rest }) => rest);
      },
      async post(slug, all) {
        const p = mem.posts.get(slug);
        if (!p) return null;
        return all || p.published ? p : null;
      },
      async upsertPost(p) {
...
app.post("/api/v1/posts", requireAdmin, async (req, res) => {
  ...
  if (!String(b.title || "").trim()) return res.status(400).json({ error: "lipsește titlul" });
